...
class Helper {
    ...
    func formatTime(open: [Substring], close: [Substring], lb: Bool) -> String {
        ...
        var ohour = String(Int(open[0])!) //get rid of leading zeros
        ...
        var chour = String(Int(close[0])!) //get rid of leading zeros
    ...
    func openNow(open: [Substring], close: [Substring]) -> Bool {
        ...
        if (components.hour! < open_hour) {
            ...
            if (components.hour! < close_hour) { return true }
            else if (components.hour! == close_hour && components.minute! >= open_minute && components.minute! <= close_minute) { return true }
            return false
        } else if (components.hour! > open_hour) {
            if (components.hour! < close_hour) { return true }
            else if components.hour! > close_hour { return false}
            else if components.minute! <= close_minute { return true }
            else { return false }
        }
